  ...
  switch (user?.status) {
      ...
      To get started, please connect your Twitter account.`,
        {
          reply_markup: InlineKeyboard.keyboard([
            [connect_twitter({ user }), view_active_campaigns({ user })],
            [how_it_works({ user })],
          ]),
          parse_mode: "markdown",
        }
    ...
    case "ADDED_TWITTER": {
      await context.reply(
        ...
        {
          reply_markup: InlineKeyboard.keyboard([
            [verify_account({ user }), change_twitter({ user })],
          ]),
        }
      );
    ...
    case "VERIFYING_TWITTER": {
      await context.reply(
        ...
        {
          reply_markup: InlineKeyboard.keyboard([
            [confirm_verification({ user }), change_twitter({ user })],
          ]),
        }
      );
    ...
    case "VERIFIED": {
      await context.reply(
        `🎉 Welcome back, @${user.twitter_username}! Your Twitter account is verified.\n\nYou can now:\n- Add new engagement campaigns 📢\n- Join existing campaigns 💬\n- Check your points and progress 🧮`,
        {
          reply_markup: InlineKeyboard.keyboard([
            [add_campaign({ user })],
            [check_points({ user }), my_points({ user })],
...
export const handleAddTwitter = async function (
  ...
  if (usedBy.rowCount && usedBy.rows[0].user_id === user.user_id) {
    context.reply(
      ...
      {
        reply_markup: InlineKeyboard.keyboard([
          ...(usedBy.rows[0].twitter_username_verified
            ? [verify_account({ user })]
            : []),
          change_twitter({ user }),
        ]),
